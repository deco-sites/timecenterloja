...
function CartItem({ index, currency }: Props) {
  ...
  return (
    <div class="pb-3 flex flex-row justify-between items-start gap-4 border-solid border-b-[1px] border-[#F7F7F7]">
      <div class="bg-[#f6f6f6] min-w-[100px] rounded-md">
        <Image
          src={imageUrl}
          alt={skuName}
          width={60}
      ...
      <div class="flex-grow">
        <span class="uppercase line-clamp-2 font-bold text-xs text-base-content lg:text-xs">
          {name}
        </span>
        <div class="flex items-center gap-2">
          {!(isGift || sellingPrice == listPrice) &&
            (
        ...
        {installmentOptions?.installments.length && !isGift
          ? (
            <div>
        ...
        <div class="flex justify-between mt-2.5 items-center">
          ...
          <Button
            ...
            class="btn btn-ghost"
          >
            <Icon class="text-accent" id="Trash" width={20} height={20} />
          </Button>
        </div>
      </div>
    </div>
