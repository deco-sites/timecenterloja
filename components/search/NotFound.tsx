...
export default function NotFound({ termDescription = "Resultados de busca encontrados para:", title = "Ops!", hints, searchTerm, shelf, }: SectionProps<typeof loader>) {
    return (<div class="w-full">
      <div class="flex flex-col gap-[10px] lg:flex-row lg:justify-around lg:mt-[30px] lg:mb-[60px]">
        ...
        {hints && (<div class="bg-primary p-5 flex flex-col gap-[10px] text-white rounded-[20px] lg:relative lg:justify-center lg:p-[60px] lg:rounded-r-none">
            ...
            <ul class="flex flex-col gap-3 list-disc">
              {hints.hints.map((hint) => (<li class="text-xs lg:text-sm  flex items-center gap-2">
                  <Icon height={20} width={20} id="ChevronRight" class="text-info"/>
                  <span>{hint}</span>
                </li>))}
            </ul>
          </div>)}
