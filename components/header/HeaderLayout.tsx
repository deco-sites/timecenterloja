...
function HeaderLayout({ navItems = megaMenuDefaultItems as INavItem[], minicart, searchbar, logo, device, }: SectionProps<ReturnType<typeof loader>>) {
    return (<header class="z-50 lg:p-0 py-2">
      <div class="flex justify-between items-center lg:p-0">
        <div class="flex items-center gap-5">
          ...
          <a href="/" class="" aria-label="Store logo">
            ...
            {/* <Logo /> */}
            {(logo?.image) && (<Image src={logo?.image} width={logo?.width ?? 91} height={logo?.height ?? 17} alt={logo?.alt ?? "Logo"} title={logo?.alt ?? "Logo"}/>)}
          </a>
        </div>
