...
function LgpdModal({ isOpen, text, button, modalCloseExpiredDate, }: SectionProps<ReturnType<typeof loader>>) {
    ...
    return (<>
      <dialog ref={modalRef} class="modal-euro bg-secondary bg-opacity-70">
        <div class="overflow-visible px-10 py-2 mb-5 self-end flex flex-row items-center max-w-6xl rounded-2xl bg-base-100">
          <div dangerouslySetInnerHTML={{ __html: text }} class="text-[13px] text-center lg:pr-0 "/>
          <button onClick={() => {
            modalRef.current?.close();
            setCookieOnCloseModal("closed", modalCloseExpiredDate);
        }} aria-label="Fechar" style={{
            minWidth: "150px",
        }} class={`capitalize md:ml-5 mt-2.5 min-h-12 h-12 font-semibold btn rounded-full join-item btn-${BUTTON_VARIANTS[button?.variant as string] ||
