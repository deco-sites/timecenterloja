...
function Result({ page, variant, layout, hideFilter: hideFilters, highlights, startingPage = 0, }: Omit<Omit<Props, "page">, "notFoundSection"> & {
    ...
    const productsFound = (<h6 class="text-secondary uppercase font-medium text-base">
      {pageInfo.records} Produtos encontrados
    </h6>);
    return (<>
      <div>
        <div class="flex flex-row gap-8">
          {variant === "aside" && newFilters.length > 0 && (<aside class="hidden lg:block w-min mt-1 min-w-[270px]">
          ...
          <div class="flex flex-col gap-5 w-full max-w-auto lg:max-w-[calc(100%-270px)]">
            ...
            <div class="flex-grow">
              <ProductGallery products={products} layout={layout} highlights={highlights}/>
              {/* <SearchPagination pageInfo={pageInfo} /> */}
              {(nextPage || previousPage) && (<Pagination pageInfo={pageInfo} productsLength={products.length} startingPage={startingPage}/>)}
            </div>
          </div>
        </div>
      </div>
...
function SearchResult({ page, notFoundSection: { Component: NotFoundSection, props: notFoundProps } = {
    Component: NotFound,
    props: {},
}, ...props }: Props) {
